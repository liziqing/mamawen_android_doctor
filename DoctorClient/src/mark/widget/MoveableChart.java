...
public class MoveableChart extends View {
	...
	private int padding_left=0,padding_top=0,padding_right=0,padding_bottom=60;
	...
	private void dataToPoint() {
		...
		for(int i=0;i<40;i++){
			...
			p.y = (float) (chartBottom - Math.random() *(chartBottom-chartTop));
			data[i] = p;
		}
	...
	private float getYScale(float value){
		return (value-minYValue)/(maxYValue-minYValue);
	}
